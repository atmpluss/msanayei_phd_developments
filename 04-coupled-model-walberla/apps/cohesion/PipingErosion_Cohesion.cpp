...
int main( int argc, char **argv )
{
   ...
   real_t sphereRadius_SI = diameter / 2_r;
   real_t sphereVolume_SI = 4_r/ 3_r * math::pi * sphereRadius_SI * sphereRadius_SI * sphereRadius_SI;
   real_t sphereMass_SI = particleDensityRatio * densityFluid * sphereVolume_SI;
   ...
   real_t kn = kn_SI / ( densityFluid * dx_SI * dx_SI * dx_SI / ( dt_SI * dt_SI ));
   real_t nun = nun_SI / ( densityFluid * dx_SI * dx_SI * dx_SI / ( dt_SI ));
   real_t y_n = y_n_SI / ( densityFluid * dx_SI * dx_SI * dx_SI * dx_SI  / ( dt_SI * dt_SI ));
