//
// Created by mohammad sanayei on 28.04.22.
//
...
void recordBoxPosition(data::ParticleAccessorWithShape & accessor, walberla::id_t boxUID, std::string fileName){
   auto boxIdx = accessor.uidToIdx(boxUID);
   WALBERLA_CHECK(boxIdx != accessor.getInvalidIdx());
   auto boxPosition = accessor.getPosition(boxIdx);
   std::ofstream file;
   file.open( fileName.c_str() );
   file  << boxPosition[0] << " " << boxPosition[1] << " " << boxPosition[2] << "\n";
   file.close();
} 
...
int main( int argc, char ** argv )
{
   ...
   //import spheres
   initSpheresFromFile("logging_spheres_particleBed.txt", *ps, *mesapdDomain, sphereShape);
   ...
   Vector3<real_t> boxPositions = (Vec3{centerPoint[0], centerPoint[1], centerPoint[2] + 70});

   auto boxShape = ss->create<data::Box>(boxEdgeLength);
   ss->shapes[boxShape]->updateMassAndInertia(densityParticle);
   Vector3<real_t> boxVelocity(real_t(0),real_t(0),real_t(-10)/dx_SI * dt_SI);
   ...
   auto sphereVtkWriter = walberla::vtk::createVTKOutput_PointData(sphereVtkOutput, "spheres", 1, vtkOutputFolder, "simulation_step");
   ...
   auto boxVtkWriter = walberla::vtk::createVTKOutput_PointData(boxVtkOutput, "box", 1, vtkOutputFolder, "simulation_step");
   ...
   for(uint_t t = 0; t < numberOfTimeSteps; ++t) {
      ...
      if(t % vtkSpacing == 0){
         WALBERLA_LOG_INFO_ON_ROOT("Created Box:  " << boxEdgeLength[0] <<" "<<boxEdgeLength[1]<<" "<<boxEdgeLength[2]);
         WALBERLA_LOG_INFO_ON_ROOT("position Box:  " << boxPositions[0] <<" "<<boxPositions[1]<<" "<<boxPositions[2]);
         WALBERLA_LOG_INFO_ON_ROOT( "Timestep " << t << " / " << numberOfTimeSteps );
         recordBoxPosition(*accessor, boxUID,"logging_boxPositiion.txt");
      }
      ...
      updateBoxPosition(*accessor, boxUID, dt_SI);
      
      syncNextNeighbors(*ps, *mesapdDomain);

   }
   recordBoxPosition(*accessor, boxUID,"logging_boxPositiion.txt");
   writeSpherePropertiesToFile(*accessor, "logging_spheres_particleBedAndBox.txt", sphereShape);
   return EXIT_SUCCESS;
}
