...
class MyBoundaryHandling
{
 public:
   ...
   Type * operator()( IBlock* const block, const StructuredBlockStorage* const storage ) const
   {
      ...
      WALBERLA_LOG_INFO_ON_ROOT("domainBB " << domainBB << " !!!");
      ...
      domainBB.yMax() += cell_idx_c( FieldGhostLayers );
       ...
       CellInterval bottomPart( 50, 30, domainBB.zMin(), 150, 70, domainBB.zMin() );

       storage->transformGlobalToBlockLocalCellInterval( bottom, *block );
       storage->transformGlobalToBlockLocalCellInterval( bottomPart, *block );

       handling->forceBoundary( noslip, bottom );
       handling->forceBoundary( inflow, bottomPart );
...
int main( int argc, char **argv )
{
   ...
   real_t krFactors = 0.001;
   real_t koFactors = 0.001;
   real_t nusFactor = 0;
   real_t nurFactor = 0;
   real_t nuoFactor = 0;
   real_t y_sFactor = 0.5;
   real_t y_rFactor = 0.00001;
   real_t y_oFactor = 0.00001;
   real_t y_n_SI = 0.0002;
   real_t frictionCoefficient = 0.3;

   real_t sphereRadius_SI = particleDiameter_SI / 2_r;
   ...
   //import spheres
   initSpheresFromFile("logging_spheres_DiffBoundaryParticleBed.txt", *ps, *rpdDomain, sphereShape);
   ...
   mesa_pd::kernel::Cohesion cohesionKernel(1);
                               ...
                               [&](const size_t idx1, const size_t idx2){
                                  ...
                                  if(accessor->getShape(idx1)->getShapeType() == mesa_pd::data::Sphere::SHAPE_TYPE &&
                                     ...
                                     if (double_cast(idx1, idx2, *accessor, acd, *accessor)) {
                                        // particles overlap
                                        if (contactFilter(acd.getIdx1(), acd.getIdx2(), *accessor, acd.getContactPoint(),  *rpdDomain))
                                        {
   ...
   //mesa_pd::mpi::SyncNextNeighbors (*ps, *rpdDomain);
      ...
      for(auto subCycle = uint_t(0); subCycle < numberOfParticleSubCycles; ++subCycle )
      {
         ...
         ps->forEachParticlePairHalf(useOpenMP, mesa_pd::kernel::ExcludeInfiniteInfinite(), *accessor,
                                     [&](size_t idx1, size_t idx2){
                                        ...
                                        if (contactFilter(idx1, idx2, *accessor, filteringPoint, *rpdDomain))
                                        {
